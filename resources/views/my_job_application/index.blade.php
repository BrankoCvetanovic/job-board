        ...
        <div class="flex items-center justify-between text-xs text-slate-500">
          ...
          <div>
            <form action="{{ route('my-job-applications.destroy', $application) }}" method="POST">
              @csrf
              @method('DELETE')
              <button class="w-full rounded-md border border-slate-300 bg-red-100 px-2.5 py-1.5 text-center text-xsm font-semibold text-black shadow-sm hover:bg-red-200 ">Cancel</button>
            </form>
          </div>
        </div>
      </x-job-card>
